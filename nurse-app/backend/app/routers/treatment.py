...
def get_treatments(skip: int = 0, limit: int = 100, db: Session = Depends(get_db), current_user = Depends(get_current_user)):
    """処置のリストを取得する"""
    treatments = db.query(TreatmentModel).offset(skip).limit(limit).all()
    return treatments

@router.get("/{treatment_id}", response_model=Treatment)
def get_treatment(treatment_id: str, db: Session = Depends(get_db), current_user = Depends(get_current_user)):
    """指定されたIDの処置情報を取得する"""
...
def update_treatment(treatment_id: str, treatment: TreatmentUpdate, db: Session = Depends(get_db), current_user = Depends(get_current_user)):
...
def complete_treatment(treatment_id: str, completion_data: TreatmentComplete, db: Session = Depends(get_db), current_user = Depends(get_current_user)):
    db_treatment = db.query(TreatmentModel).filter(TreatmentModel.id == treatment_id).first()
    if db_treatment is None:
        raise HTTPException(status_code=404, detail="処置が見つかりません")
